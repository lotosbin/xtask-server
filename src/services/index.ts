...
interface IIssueAssignedTo {
  id: string;
}

export interface IIssue {
  assigned_to?: IIssueAssignedTo;
}

export async function getRedmineIssues({ host, key }: IHost, { offset, limit, project_id, issue_id, assigned_to_id }): Promise<[IIssue]> {
...
export async function issue_update({ host, key }: IHost, { issue_id, start_date, due_date, status_id }) {
  const log = message => console.log(`issue_update:${message}`);
...
export async function getRedmineProjects({ host: redmine_api_host, key: redmine_api_key }, args) {
  ...
  let response: TResponse = await fetch(`${base}?offset=${toQuery(args)}`, {
    headers: headers,
  });
