...
export class TaskTypeResolver {
    ...
    @ResolveProperty()
    async author(@Parent() parent: any, @Context() {repository}) {
        return await repository.getUserById(parent.author?.id);
    }

    @ResolveProperty()
    async assigned_to_name(@Parent() parent: any) {
        return (parent.assigned_to || {}).name;
    }

    @ResolveProperty()
    async assigned_to(@Parent() parent: any, @Context() {repository}) {
...
class ProjectTypeResolver {
    ...
    async issues(@Parent() parent: any, @Args() args, @Context() {repository}: IContext) {
        return await repository.getRedmineIssues({...args, project_id: parent.id})
    }

    @ResolveProperty()
    async memberships(@Parent() parent: any, @Args() args, @Context() {repository}: IContext) {
        return await repository.getMemberShipsByProjectId(parent.id, {...args, project_id: parent.id})
    }
}
...
class MemberShipType {
    @ResolveProperty()
    async user(@Parent() parent: any, @Args() args, @Context() {repository}: IContext) {
