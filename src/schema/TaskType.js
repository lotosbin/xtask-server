...
export const TaskType = new GraphQLObjectType({
    ...
    fields: () => ({
        ...
        project_id: {type: GraphQLString, deprecationReason: "use project.name",},
        project_name: {type: GraphQLString, deprecationReason: "use project.name", resolve: (t) => t.project.name},
        status_name: {type: GraphQLString, deprecationReason: "use project.name", resolve: (t) => t.status.name},
        status: {type: IssueStatusType, resolve: t => t.status},
        author_name: {type: GraphQLString, deprecationReason: "use author.name", resolve: (t) => (t.author || {}).name},
        author: {
            type: UserType,
            args: {},
            resolve: async (t, args, {repository}) => await repository.getUserById(t.author?.id),
        },
        assigned_to_name: {type: GraphQLString, deprecationReason: "use assigned_to.name", resolve: (t) => (t.assigned_to || {}).name},
        assigned_to: {
            type: UserType,
            args: {},
